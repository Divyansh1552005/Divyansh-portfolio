            ...
            <div className="flex flex-wrap justify-center gap-3 sm:gap-4">
              {categories.map((category) => (
                <button
                  ...
                  className={`relative px-6 sm:px-8 py-2.5 sm:py-3 rounded-full font-medium text-sm sm:text-base transition-all duration-300 overflow-hidden focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-[#0b0b0b] hover:scale-105 active:scale-95 ${activeCategory === category.id
          ...
          <div key={activeCategory} className="space-y-8">
            {filteredProjects.map((project) => (
              <article key={project.id} className="group">
                {/* Horizontal Card */}
                <div className="relative bg-gray-50 dark:bg-[#111111] backdrop-blur-sm rounded-2xl border border-gray-200 dark:border-gray-800/50 overflow-hidden transition-all duration-300">
                  <div className="relative flex flex-col md:flex-row">
                    {/* Project Image - Left */}
                    <div className="relative md:w-2/5 lg:w-1/3 flex-shrink-0 p-4 sm:p-6 flex items-center justify-center">
                      <div className="relative w-full aspect-video rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-950/50 border border-gray-200 dark:border-gray-800/30">
                        <img
                          src={project.image}
                          alt={project.title}
                    ...
                    <div className="flex-1 p-4 sm:p-6 flex flex-col">
                      ...
                      <div className="flex flex-row items-center justify-between gap-3 sm:gap-4 mb-3 w-full">
                        <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 dark:text-white leading-tight flex-1">
                          {project.title}
                        </h3>
                        ...
                        <div className="flex flex-row items-center gap-2 sm:gap-3 flex-shrink-0">
                          {project.github !== "#" && (
                            <a
                              ...
                              className="px-3 sm:px-4 py-1.5 sm:py-2 flex items-center justify-center gap-2 rounded-lg bg-black dark:bg-gray-800/50 border border-black dark:border-gray-700 text-white dark:text-gray-400 hover:text-white dark:hover:text-white hover:bg-gray-800 dark:hover:bg-gray-700 hover:border-gray-800 dark:hover:border-gray-600 hover:-translate-y-0.5 hover:scale-105 active:scale-95 transition-all duration-300 text-xs sm:text-sm font-medium"
                      ...
                      <div className="flex flex-wrap gap-2 mt-auto">
                        {project.tags.map((tag, tagIndex) => (
                          <span
                            key={tagIndex}
                            className="text-xs px-2.5 py-1 bg-blue-500/10 border border-blue-500/30 rounded-full text-blue-600 dark:text-blue-400 font-medium hover:border-blue-400/50 hover:bg-blue-500/20 transition-all duration-300"
                          >
                            {tag}
                          </span>
                        ))}
